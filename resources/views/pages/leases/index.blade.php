            ...
            <tbody>
                @forelse ($leases as $index => $lease)
                    <tr>
                        ...
                        <td>Rp.{{ number_format($lease->total_iuran, 2) }}</td>
                        <td>
                            {{-- Edit Modal --}}
                            <button type="button" class="btn btn-warning" data-bs-toggle="modal"
                                data-bs-target="#editModal{{ $lease->id }}">Edit</button>
                                            ...
                                            <form action="{{ route('leases.update', $lease->id) }}" method="POST">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <select class="form-select" name="property_id"
                                                        id="editProperty{{ $lease->id }}" required>
                                                        @foreach ($properties as $property)
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="date" class="form-control" name="end_date"
                                                        ...
                                                        value="{{ \Carbon\Carbon::parse($lease->end_date)->format('d/m/Y') }}"
                                                        required>
                                                </div>
                                                <div class="mb-3">
                                                    ...
                                                    <select class="form-select" name="status"
                                                        id="editStatus{{ $lease->id }}" required>
                                                        <option value="active"
                                                            {{ $lease->status == 'active' ? 'selected' : '' }}>Active
                                                        </option>
                                                        <option value="inactive"
                                                            {{ $lease->status == 'inactive' ? 'selected' : '' }}>Inactive
                                                        </option>
                                                        <option value="pending"
                                                            {{ $lease->status == 'pending' ? 'selected' : '' }}>Pending
                                                        </option>
                                                ...
                                                <div class="mb-3">
                                                    <label for="editTotalIuran{{ $lease->id }}"
                                                        class="form-label">Total Iuran:</label>
                                                    <input type="number" class="form-control" name="total_iuran"
                                                        id="editTotalIuran{{ $lease->id }}"
                                                        value="{{ $lease->total_iuran }}" step="0.01" required>
                                                </div>
                                                <div class="modal-footer">
